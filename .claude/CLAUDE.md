...
このリポジトリには `oop/` ディレクトリがあり、オブジェクト指向の練習問題が格納されている。
セッションが新しくなった場合は、以下の手順で学習を継続すること。

### ディレクトリ構成

```
oop/
├── README.md           # 全体の説明・進捗一覧
├── exercises/          # 練習問題
│   └── XX_xxx/
│       ├── README.md   # 問題文
│       ├── *.rb        # 学習者の解答
...
1. **問題を出す**: `exercises/XX_xxx/README.md` と `spec/XX_xxx/*_spec.rb` を作成
2. **考えさせる**: コードを書く前に、設計上の問いを投げかける
3. **実装を待つ**: 学習者がコードを書いてプッシュするのを待つ
4. **コードを読む**: プッシュされたら `git pull` してコードを確認
5. **質疑応答**: 「なぜそう書いた？」を聞き、理解度を確認する
6. **レビュー作成**: レッスン終了時に `REVIEW.md` を作成
...
`oop/README.md` のレッスン一覧テーブルで進捗を管理する。レッスン完了時に状態を更新すること。

### 注意事項

- 答えを直接教えない。考えさせる問いを投げる
- コードが動くかどうかだけでなく「なぜそう書いたか」を重視する
- 理解が曖昧な場合は、追加の質問で深掘りする
- 各レッスン終了時に必ず REVIEW.md を作成する

---

## Web基礎学習リポジトリの進め方

このリポジトリには `web/` ディレクトリがあり、Webエンジニアリングの基礎を学ぶコンテンツが格納されている。
**コードを書かずに概念理解を深める形式**のため、スマートフォンでも学習可能。
...
└── lessons/
    └── XX_topic_name/
        ├── README.md      # レッスン本文（読み物 + 確認問題）
        └── REVIEW.md      # 学習記録（完了後に作成）
```
...
| # | テーマ | 内容 |
|---|--------|------|
| 01 | Same-Origin Policy と CORS | ブラウザセキュリティの土台 |
| 02 | 認証と認可の違い | 基本概念の明確化 |
| 03 | セッション認証の仕組み | Cookie、CSRF対策 |
| 04 | トークン認証と JWT | セッションとの比較 |
| 05 | OAuth 2.0 / OIDC | 外部認証の仕組み |
| 06 | キャッシュの基礎 | HTTPキャッシュヘッダー |
| 07 | CDN とキャッシュ無効化 | 実務でハマるポイント |
| 08 | N+1問題とインデックス | パフォーマンスの基本 |
| 09 | 冪等性と非同期処理 | API設計の重要概念 |
| 10 | CAP定理と結果整合性 | 分散システムの考え方 |

### 学習フロー

1. **レッスンを読む**: `lessons/XX_topic/README.md` を読む
2. **確認問題に答える**: レッスン末尾の質問にチャットで回答
3. **質疑応答**: 回答に対して深掘り質問を行い、理解度を確認
4. **REVIEW.md作成**: レッスン終了時に学習記録を作成
5. **進捗更新**: `web/README.md` のテーブルを更新
...
- **コードを書かない**: 概念理解と言語化を重視
- **質問形式**: 選択式・記述式の確認問題
- **深掘り対話**: 回答が浅い場合は追加質問で掘り下げる
- **関連付け**: 概念同士の関係（例: Same-Origin Policy と CSRF）を理解させる

### 注意事項

- 正解をすぐに教えない。まず考えさせ、回答を引き出す
- 間違いは否定せず、「惜しい」「方向は合ってる」など段階的に導く
- 「なぜそうなっているか」の歴史的経緯や設計意図も伝える
- 実務でハマりやすいポイント、よくある誤解を積極的に扱う
